...
// 功能: 读取umd文件，通过方法获取书名等信息以及章节数，章节名和内容
...
func (umd *UMDReader) splitFilePath() error {
	...
	umd.UMDDir = outDir
	umd.UMDNameNoExt = strings.TrimSuffix(umdName, filepath.Ext(umdName))
	return nil
}

func (umd *UMDReader) readUMD() error {
	bytesUMD, err := tool.ReadFile(umd.UMDPath) // 一次性读入
	if err != nil {
	...
	for {
		...
		if 35 == block[0] {
			...
			case 12: // 0x0C 文件结束:整个文件长度
				...
				// 0x2920 -> 0x0A00
				contentBytes = bytes.Replace(buffer.Bytes(), []byte{0x29, 0x20}, []byte{0x0A, 0x00}, -1)
				log.Println("- 正文块 Len:", len(contentBytes), "应该长度:", contentLen)
				// tool.WriteFile("T:/content.bin", contentBytes[:contentLen], 0666)

				umd.InfoDate = strYear + "-" + strMonth + "-" + strDay
				if bHaveCover {
					umd.CoverPath = filepath.Join(umd.UMDDir, "cover.jpg")
					tool.WriteFile(umd.CoverPath, mapIDContent[idCover], 0666)
				}
